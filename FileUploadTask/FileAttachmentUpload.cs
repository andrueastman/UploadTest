...
{
    public static class FileAttachmentUpload
    {
        private static async Task<IUploadSession> CreateFileAttachmentUploadSession(IBaseClient graphClient, string messageId, long streamLength)
        {
            ...
            string attachmentInfo = $"{{\"AttachmentItem\": {{\"attachmentType\": \"file\",\"name\": \"flower\",\"size\": {streamLength} }}}}";
            HttpRequestMessage httpRequestMessage = new HttpRequestMessage(HttpMethod.Post, uri)
            {
                Content = new StringContent(attachmentInfo, Encoding.UTF8, "application/json")
            };
